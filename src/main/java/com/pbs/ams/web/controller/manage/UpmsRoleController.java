...
public class UpmsRoleController extends BaseController {
    ...
    public String permission(@PathVariable("id") long id, ModelMap modelMap) { // long id, int id
    ...
    @ResponseBody
    public Object permission(@PathVariable("id") long id, HttpServletRequest request) {// long id, int id
    ...
    @ResponseBody
    public Object update(@PathVariable("id") long id, UpmsRole upmsRole) {// long id, int id
        ComplexResult result = FluentValidator.checkAll()
                .on(upmsRole.getName(), new LengthValidator(1, 20, "名称"))
                .on(upmsRole.getTitle(), new LengthValidator(1, 20, "标题"))
                .doValidate()
