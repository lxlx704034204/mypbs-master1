...
$(function() {
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('pbs-ams-management-systemid', systemid);
		$.cookie('pbs-ams-management-systemname', systemname);
		$.cookie('pbs-ams-management-systemtitle', systemtitle);
	});
	// 显示cookie菜单
	var systemid = $.cookie('pbs-ams-management-systemid') || 1;
	var systemname = $.cookie('pbs-ams-management-systemname') || 'pbs-ams-management-server';
	var systemtitle = $.cookie('pbs-ams-management-systemtitle') || '权限管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);
